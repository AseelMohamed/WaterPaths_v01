...
namespace Constants {
    ...
    // Experiment 1: 4 objs, reliability=MIN (worst case)
    ...
    enum AggregationMethod {
        MIN_WORST_CASE = 0,  // Take minimum (for reliability) or maximum (for affordability)
        AVERAGE = 1           // Take average across all WSS
    };
    
    int getNumObjectives();  // Dynamic number of objectives based on experiment mode
...
//#define NUM_DEC_VAR 57;
    ...
    const double WEEKS_IN_YEAR = 52.0;
    ...
    inline AggregationMethod getAffordabilityAggregationMethod() {
        // Experiment 3: MAX (worst case)
        // Experiment 4: AVERAGE
        return (EXPERIMENT_MODE == 4) ? AVERAGE : MIN_WORST_CASE;
    }
