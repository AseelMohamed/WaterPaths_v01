...
WSSDataCollector::WSSDataCollector(const WaterSupplySystems *wss, unsigned long realization)
    ...
    initial_households = wss->getInitialHouseholds();
}
...
void WSSDataCollector::collect_data() {
    ...
    // Also fail if there is any unfulfilled demand this week
    if (wss->getUnfulfilled_demand() > 0.0) {
        week_failed = true;
    }
...
double WSSDataCollector::getInitial_households() const {
