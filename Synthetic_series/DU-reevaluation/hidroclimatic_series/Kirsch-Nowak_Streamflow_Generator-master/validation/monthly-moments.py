...
# Make statistical validation plots of monthly moments for each water source
space = ['real','log']
legend_loc = ['upper right','lower left']
site = 'qDescoberto' #here it should be changed for each water source
H = np.loadtxt('historical/' + site + '-monthly.csv', delimiter=',') # n_historical_years x 12
S = np.loadtxt('synthetic/' + site + '-1000x40-monthly.csv', delimiter=',') # n_realizations x 12*n_synthetic_years
